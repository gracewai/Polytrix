...
var downloadFileLink = function(location,access_token,refresh_token,res){
	...
	return Q.Promise(function(resolve,reject){
		request.get({
			url:'https://www.googleapis.com/drive/v2/files/' + location,
			headers:{
	...
	.then(function(okString){
		return Q.Promise(function(resolve,reject){
			request.post({
					url:'https://www.googleapis.com/drive/v2/files/' + metadata.id + '/permissions',
					headers:{
						Authorization: "Bearer " + access_token
					},
					json: {
				...
				function(error,response,body){
					if(!error){
						if(response.statusCode == 200){
							console.log('permission metadata:');
							console.log(body);
							resolve(body.id);
						}else{
							console.log('response.statusCode: ' + response.statusCode);
	...
	.then(function(permissionId){
		...
		setTimeout(function(){
			request.del({
				url:'https://www.googleapis.com/drive/v2/files/' + metadata.id + '/permissions/' + permissionId,
				headers:{
					Authorization: "Bearer " + access_token
				}},
				function(error,response,body){
					...
					if(!error){
						if(response.statusCode == 204){
							console.log('delete permission return: OK');
						}else{
							console.log('response.statusCode: ' + response.statusCode);
							console.log(body);
						}
